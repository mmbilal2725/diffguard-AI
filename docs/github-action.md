...
DiffGuard-AI can run as a GitHub Action on pull requests. The action fetches the PR, runs the review pipeline, reads `.diffguard-rules.md` from the PR head when present, posts one summary comment, and stores the run only when `DATABASE_URL` is available.
...
Use `--dry-run` to fetch and review the PR without posting a GitHub comment. If `DATABASE_URL` is not set, DiffGuard-AI skips database persistence.
